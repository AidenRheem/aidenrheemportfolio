...
export class PocketShader {
  ...
  _builtinUniformLocations = new Map();
    ...
    if (init) {
      if (typeof globalThis.window === 'undefined') {
        ...
        if (!!import.meta?.env?.DEV) {
          console.warn('PocketShader is not running in a browser environment.  Aborting automatic initialization.');
        }
        return;
      }
  ...
  /**
    ...
    // prettier-ignore
    const prepends = [
      parts[0].startsWith('#version') ? parts.shift() : '#version 300 es'
    ];
    ...
    if (this.container.clientWidth === 0 || this.container.clientHeight === 0) {
      console.error(`PocketShader container has a width or height of 0px.  The canvas will not be visible until the container has a non-zero size size.`, {
  ...
  /**
    ...
    const _render = (now)=>{
      ...
      then = now;
      this.mouseSmoothed.x += (this.mouse.x - this.mouseSmoothed.x) * (1 - this.mouseSmoothing);
      ...
      this.ctx.uniform2f(resolutionLocation, this.builtinUniforms.u_resolution[0], this.builtinUniforms.u_resolution[1]);
      this.ctx.uniform2f(mouseLocation, this.builtinUniforms.u_mouse[0], this.builtinUniforms.u_mouse[1]);
  ...
  /**
    ...
    this.mouse.y = (this._canvasRectCache.height - (e.clientY - this._canvasRectCache.top) - 1) / this._canvasRectCache.height;
  };
  /**
  ...
  /**
    ...
    this._builtinUniformLocations.set('a_position', this.ctx.getUniformLocation(this.program, 'a_position'));
    this._builtinUniformLocations.set('u_resolution', this.ctx.getUniformLocation(this.program, 'u_resolution'));
    this._builtinUniformLocations.set('u_mouse', this.ctx.getUniformLocation(this.program, 'u_mouse'));
    this._builtinUniformLocations.set('u_time', this.ctx.getUniformLocation(this.program, 'u_time'));
  ...
  _setupCanvas() {
    ...
    this.getMouseTarget().addEventListener('mousemove', this.setMousePosition);
    this.getMouseTarget().addEventListener('touchmove', this.setTouchPosition, {
      passive: false
    });
  ...
  /**
	 ...
	 */ _setUniform(ctx, location, uniform) {
  ...
  /**
	 ...
	 */ _validateUniforms(source) {
    const uniformRegex = /uniform\s+(\w+)\s+(\w+)\s*;/g;
    let match;
    while((match = uniformRegex.exec(source)) !== null){
      const [, , name] = match;
      if ([
        ...this._builtinUniformLocations.keys(),
        ...Object.keys(this.uniforms)
      ].some((k)=>k === name)) {
        continue;
      }
      throw new Error(`Uniform "${name}" is referenced in the shader, but no value was provided in PocketShaderOptions.uniforms.`);
    }
    return this;
  }
  /**
 ...
 */ function throttledDebounce(/**
 ...
 // 	}
 // }
//# sourceMappingURL=pocket-shader.js.map